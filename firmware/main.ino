//***3RPS Parallel Manipulator Ball Balancer Code BY Aaed Musa, Adapted by Jason Zurlo for UTC Project**
//--------------------------------------------------------------
//libraries
...
void checkSerial() {
  static char buf[64];  // static so it persists, big enough for all commands
  if (!Serial.available()) return;

  size_t len = Serial.readBytesUntil('\n', buf, sizeof(buf) - 1);
  buf[len] = '\0';  // null terminate
  char *cmd = strtok(buf, " ");  // first token (command/state)
  ...
  if (strcasecmp(cmd, "init") == 0) {
    myState = Initialize;
    Serial.println("State set to INITIALIZE");
    return;
  }
  if (strcasecmp(cmd, "standalone") == 0) {
    myState = Standalone;
    Serial.println("State set to STANDALONE");
    return;
  }
  if (strcasecmp(cmd, "pause") == 0) {
    myState = Pause;
    Serial.println("State set to PAUSE");
    return;
  }
  if (strcasecmp(cmd, "remote") == 0) {
    myState = Remote;
    Serial.println("State set to REMOTE");
    return;
  }

  // --- Handle PID tuning (only allowed in Pause) ---
  if (myState == Pause) {
    char *valStr = strtok(NULL, " ");
    if (valStr) {
      double val = atof(valStr);
      if (strcasecmp(cmd, "kp") == 0) {
        kp = val;
        Serial.printf("Updated kp = %.6f\n", kp);
      } else if (strcasecmp(cmd, "ki") == 0) {
        ki = val;
        Serial.printf("Updated ki = %.6f\n", ki);
      } else if (strcasecmp(cmd, "kd") == 0) {
        kd = val;
        Serial.printf("Updated kd = %.6f\n", kd);
      } else {
        Serial.println("Unknown PID parameter. Use kp, ki, kd");
      }
    } else {
      Serial.println("Invalid PID format. Use: kp <value>");
    }
    return;
  }

  // --- Handle Remote coordinates (only when in Remote mode) ---
  if (myState == Remote) {
    // Expecting: "x,y"
    char *xStr = strtok(cmd, ",");
    char *yStr = strtok(NULL, ",");
    if (xStr && yStr) {
      remX = atoi(xStr);
      remY = atoi(yStr);
      //Serial.printf("Remote target set -> X:%d Y:%d\n", remX, remY);
    } else {
      //Serial.println("Invalid coordinate format. Use: x,y");
    }
    return;
  }

  // --- If none matched ---
  //Serial.println("Unknown command or wrong state.");
}
...
void moveTo(double hz, double nx, double ny) {
  ...
  if (detected) {
   ...
   for (int i = 0; i < 3; i++) {
  double th = machine.theta(i, hz, nx, ny);
  pos[i] = round((angOrig - th) * angToStep);
}
...
void PID(double setpointX, double setpointY) {
  ...
  if (p.x != 0) {
    ...
    //Serial.println((String) "X = " + p.x + "   Y = " + p.y);  //print X and Y outputs
  }
